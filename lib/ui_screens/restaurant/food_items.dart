...
class _FoodItemsScreenState extends State<FoodItemsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if(snapshot!=null &&categoryList!=null){
      ...
      if(selectedCategory ==null) selectedCategory =categoryList.firstWhere((element) => element.id==restaurant.categoryList![0]);
    }

    return snapshot!=null&&categoryList!=null?Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Flexible(flex: 3, fit: FlexFit.tight, child: selectOrderType(categoryList)),
        Flexible(
            flex: 15,
            fit: FlexFit.tight,
            child: StreamProvider<List<FoodModel>?>.value(
                initialData: null,
                value: DatabaseService().queryLiveFood(category: selectedCategory!.id!, restaurant: restaurant.id!),child: Padding(
                  padding:  EdgeInsets.only(top: 10),
                  child: FoodListWidget(myCategory: myCategory,),
                ))),
        Flexible(
            flex: 2,
            fit: FlexFit.tight,
            child: OnClick(
              ...
              onTap: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (_) => AddNewItem(widget.restaurantId,myCategory)));
              },
              child: Container(
  ...
  Widget selectOrderType(List<CategoryModel> categoryList) {
    if(myCategory.isEmpty){
      for(String item in restaurant.categoryList!){
        //get my category
        myCategory.add(categoryList.firstWhere((element) => element.id==item));
      }
    }
    return selectedCategory !=null?Material(
      elevation: 2.0,
    ...
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ClipRRect(
              borderRadius: BorderRadius.circular(selected!=null && selected!.id==id ?30:25),
              child: Image.network(
                image,
            ...
            Text(
              title,
              style: TextStyle(
                  color: selected!=null && selected!.id==id ?Colors.redAccent:Colors.white,
...
class FoodListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return foodList!=null ?ListView.builder(
            ...
            child: RestaurantFoodItem(
                ...
                onEditTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => EditItem(foodItem: foodList[index],categoryList:myCategory,)));
                },
                currentPrice: (price) {
                  print(price!.price);
                }),
